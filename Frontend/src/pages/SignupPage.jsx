...
const InputField = ({ id, name, label, type, value, onChange, error, placeholder, icon, maxLength, required = true, focusColor = '#2563eb' }) => (
  <div>
    ...
    <div className="relative">
      ...
      <input id={id} name={name} type={type} required={required} value={value} onChange={onChange} maxLength={maxLength}
        className={`block w-full pl-10 pr-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[${focusColor}] focus:border-[${focusColor}] dark:bg-gray-700 dark:border-gray-600 dark:text-white ${error ? 'border-red-500' : 'border-gray-300 dark:border-gray-500'}`}
...
const SelectField = ({ id, name, label, value, onChange, error, icon, children, required = true, focusColor = '#2563eb' }) => (
  <div>
    <label htmlFor={id} className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{label}</label>
    <div className="relative">
      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">{icon}</div>
      ...
      <div className="max-w-2xl w-full space-y-6 p-8 md:p-10 bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputField id="fullName" name="fullName" label="Full Name" type="text" value={formData.fullName} onChange={handleInputChange} error={errors.fullName} placeholder="Enter your full name" icon={<User className="h-5 w-5 text-gray-400" />} focusColor={focusColor} />
          </div>

          <InputField id="email" name="email" label="Email Address" type="email" value={formData.email} onChange={handleInputChange} error={errors.email} placeholder="you@student.cuet.ac.bd" icon={<Mail className="h-5 w-5 text-gray-400" />} focusColor={focusColor} />

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <SelectField id="hall" name="hall" label="Hall" value={formData.hall} onChange={handleInputChange} error={errors.hall} icon={<Building className="h-5 w-5 text-gray-400" />} focusColor={focusColor}>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className={`block w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[${focusColor}] focus:border-[${focusColor}] dark:bg-gray-700 dark:border-gray-600 dark:text-white`}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="password" name="password" type={showPassword ? 'text' : 'password'} required value={formData.password} onChange={handleInputChange}
                  className={`block w-full pl-10 pr-10 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[${focusColor}] focus:border-[${focusColor}] dark:bg-gray-700 dark:border-gray-600 dark:text-white ${errors.password ? 'border-red-500' : 'border-gray-300 dark:border-gray-500'}`}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="confirmPassword" name="confirmPassword" type={showConfirmPassword ? 'text' : 'password'} required value={formData.confirmPassword} onChange={handleInputChange}
                  className={`block w-full pl-10 pr-10 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[${focusColor}] focus:border-[${focusColor}] dark:bg-gray-700 dark:border-gray-600 dark:text-white ${errors.confirmPassword ? 'border-red-500' : 'border-gray-300 dark:border-gray-500'}`}
          ...
          <div>
            <button type="submit" disabled={isSubmitting}
              className={`${primaryButton} group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium focus:ring-2 focus:ring-offset-2 transition-all duration-200 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}>
              {isSubmitting ? (
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              ...
              <Link to="/login" className={`font-medium ${primaryText} hover:${primaryText}/80 transition-colors duration-200`}>Sign in</Link>
            </p>
          </div>
        </form>
      </div>
